...
class SEANModel(torch.nn.Module):
    @staticmethod
    ...
    def forward(self, data, mode):
        input_semantics, input_semantics_onehot, real_image = self.preprocess_input(data)

        if mode == 'generator':
            g_loss, generated = self.compute_generator_loss(
                input_semantics, input_semantics_onehot, real_image)
            ...
            if self.opt.label_type == 'edge':
                style_matrix = self.netE(real_image)
            else:
                style_matrix = self.netE(real_image, input_semantics)
            # z, mu, logvar = self.encode_z(real_image)
            return style_matrix
        elif mode == 'inference':
            with torch.no_grad():
                fake_image = self.generate_fake(input_semantics, input_semantics_onehot, real_image)
            return fake_image
        else:
    ...
    def compute_generator_loss(self, input_semantics, input_semantics_onehot, real_image):
        G_losses = {}
        ...
        pred_fake, pred_real = self.discriminate(
            input_semantics_onehot, fake_image, real_image)
    ...
    def compute_discriminator_loss(self, input_semantics, input_semantics_onehot, real_image):
        D_losses = {}
        # with torch.no_grad():
        fake_image = self.generate_fake(input_semantics, input_semantics_onehot, real_image)
