...
from models.networks.normalization import SPADE, STYLE

# os.environ['TORCH_HOME'] = 'E://pku/models/'
...
class STYLEResnetBlock(nn.Module):
    ...
    def forward(self, x, seg, st):
        ...
        # if torch.isinf(st_1).any():
        #     raise AssertionError("inf in st_1")
        # if torch.isnan(st_1).any():
        #     raise AssertionError("nan in st_1")
...
class ResnetBlock(nn.Module):
    def __init__(self, dim, norm_layer, activation=nn.ReLU(inplace=True), kernel_size=3):
        super().__init__()
