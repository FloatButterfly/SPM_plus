...
from .EntropyBottleneck import EntropyBottleneck, TextureEntropyBottleneck_no_hyperprior
...
class CSEANModel(torch.nn.Module):
    ...
    def forward(self, data, mode='generator', style_code=None):
        ...
        if mode == 'transfer':
            ...
            style_code_hat, texture_length = self.texture_entropy(style_code/1e-2)
        ...
        style_matrix = style_matrix / 1e-2  # 避免量化之后都为0的情况
        # downsampling = 0 in texture entropy, only project to low dimension in channel, no need for padding
        style_matrix_hat, texture_length = self.texture_entropy(style_matrix)
