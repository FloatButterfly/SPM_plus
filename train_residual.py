...
from trainers.residual_trainer import End2EndTrainer
from util import *
...
# have to set lmbda, k_mse, k_lpips
...
class WithoutTest(TrainBase):
    ...
    def _print_infor(self, **kwargs):
        lr = min(param_group["lr"] for param_group in self.trainer.optimizer.param_groups)
        if self.opt.k_mse:
            psnr = 10 * np.log10(255 ** 2 / (kwargs["mse"]))
        else:
            psnr = 0.0
        self.writer.add_scalar("bpp", kwargs["bpp"], global_step=kwargs["iteration"])
        ...
        return "LR: {:>3.1e}\t Loss: {:>.4f}\t PSNR: {:>5.2f} dB\t residual_bpp:{:>.4f}\t weighted_residual_bpp:{:>.4f}\t weighted_mse {:>.4f}\t weighted_lpips {:>.4f}\t".format(
            lr, kwargs["loss"], psnr, kwargs["bpp"], kwargs["bpp"] * self.opt.lmbda, kwargs["mse"] * self.opt.k_mse,kwargs["lpips"] * self.opt.k_lpips,
        )
